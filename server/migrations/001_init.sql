...
--
-- Design notes:
--   * Single-game and deliberately so. An earlier draft carried a games table and
--     a game_id on every table; the game is standalone, so that came out. If a
--     second title ever shares these accounts, adding game_id back is a ~10-line
--     migration with a constant default -- not the painful backfill it would be
--     for a value you cannot infer.
--   * Session tokens are stored hashed. A DB leak must not be a session leak.
--   * Save blobs live in their own table so the load screen and leaderboard never
--     read them; headline stats are denormalised onto save_slots for that.
...
CREATE TABLE users (
  ...
  -- a sanity net, not a proof: an account must be reachable somehow
...
-- Present from the first migration so OAuth is a row, not a schema change.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- sessions — opaque tokens, stored as sha256(token)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- save_slots — the "file" a player names. Metadata only; the blob lives next door.
--
-- `revision` is the optimistic-concurrency token. Clients send the revision they
-- edited from; a stale write gets 409 and the conflict UI. See API-CONTRACT.md §5.
-- ---------------------------------------------------------------------------
...
CREATE INDEX save_slots_user_idx ON save_slots (user_id, updated_at DESC);
...
--
-- jsonb over bytea: Postgres TOASTs and compresses anything over ~2 KB anyway,
-- and keeping it queryable makes server-side migrations and analytics possible.
-- Expect 100-200 KB for a full 35x35 park.
--
-- The 2 MB cap is enforced in the API, not here: pg_column_size() is STABLE and
-- Postgres only accepts IMMUTABLE expressions in CHECK constraints. The API
-- boundary is the better place anyway -- a 413 with a useful message beats a
...
-- ---------------------------------------------------------------------------
-- save_history — rolling backups, so a bad autosave is recoverable and a botched
-- migration has something to replay. Pruned to N per slot by a scheduled job,
-- not by a trigger.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- scores — best value per (user, metric). Submitting a worse value must not
-- overwrite. Derived server-side from a validated save; there is no submit
-- endpoint, so a client cannot assert a score directly.
-- ---------------------------------------------------------------------------
CREATE TABLE scores (
  user_id     uuid        NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE INDEX scores_leaderboard_idx ON scores (metric, value DESC);
